...
if($_POST["tree_status"]!=2){
	...
	if(strtotime($tree_end)>=strtotime($today)){
		try {
			...
			//執行sql指令
			$sql = "update tree 
			set size = :size, amount = :amount, tree_price = :tree_price, tree_end = :tree_end, tree_update = NOW(), tree_status = :tree_status
	...
	if(strtotime($tree_end)>=strtotime($today)){
		try {
			...
			$products->bindValue(":size", $_POST["size"]);
			$products->bindValue(":amount", $_POST["amount"]);
			$products->bindValue(":tree_start",$today);
			$products->bindValue(":tree_end", $_POST["tree_end"]);
			$products->bindValue(":tree_price", $_POST["tree_price"]);
			$products->bindValue(":tree_status",$_POST["tree_status"]);
