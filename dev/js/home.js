$(function () {
  //輪播
  let settime;
  let imgheight = $('.left_con li').height();
  let picture = 1;
  $('.slideshow').css("height", `${imgheight}`); //輪播外框彈性高度(=輪播片高度)
  ...
  function show() {
    ...
    $('.circle span').eq(index - 1).show().siblings().hide();
    ...
    for (let i = 0; i <= 7; i++) {
      let number = (Math.random() * 60) - 30; //亂數0~60, 扣30 > 亂數-30~30
